...
public class WhereJoinVisitor implements ExpressionVisitor {

  private HashMap<Column, Column> joinColumns;

  public HashMap<Column, Column> GetColumnFromJoin(Expression where) {
    this.joinColumns = new HashMap<Column, Column>();
    where.accept(this);
    return joinColumns;
  }
  ...
  public void visit(EqualsTo equalsTo) {
    ...
    // skip the partition or decomposition of tables
    if (equalsTo.getLeftExpression() instanceof Column
        && equalsTo.getRightExpression() instanceof Column) {
      Column colRight = (Column) equalsTo.getRightExpression();
      Column colLeft = (Column) equalsTo.getLeftExpression();
      joinColumns.put(colLeft, colRight);
    }
  }
